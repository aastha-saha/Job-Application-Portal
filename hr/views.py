...
from hr.models import JobPost , CandidateApplications , SelectCandidateJob ,rejectCandidateJob
...
@login_required
def hrCandidateDetails1(request,id):
    if JobPost.objects.filter(id=id).exists():
        jobpost = JobPost.objects.get(id=id)
        jobapplys = CandidateApplications.objects.filter(job=jobpost)
        # print(jobapplys)
        rejectedCandidate = rejectCandidateJob.objects.filter(job=jobpost)
        print(rejectedCandidate)
        return render(request,'hr/candidate.html',{'jobapplys':jobapplys,'jobpost':jobpost,'rejectedCandidate':rejectedCandidate})
    else:
        return render('hrdash')
...
def rejectApplication(request):
    if request.method == 'POST':
        ...
        jobpostid = request.POST.get('jobpostid') 
        candidate = CandidateApplications.objects.get(id=candidateid) 
        candidate.status = 'rejected'  # Modify the status field
        candidate.delete()
        # jobpost = JobPost.objects.get(id=jobpostid)
        # if rejectCandidateJob.objects.filter(candidate=candidate).exists()==False:
        #     rejectCandidateJob(job=jobpost,candidate=candidate).save()
        # return redirect('/candidatedetails/'+str(jobpostid)+"/")
    return redirect('hrdash')